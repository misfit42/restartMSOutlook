...
def process_restart(proc: str, chk_time: int = 300, startup_time: int = 30):
    """
    ...
    while True:
        ...
        # Save as list to preserve information for later useage.
        ...
        proc_started = subprocess.run(str(pathlib.Path(proc_info[ 0 ][ 'exe' ])))
        logging.info(f'{proc_info[ 0 ][ "name" ]} has been restarted: {proc_started.args}, {proc_started.returncode}')
